 ...
 **/
 ...
 /**
  ...
  @params {Object} options The options object
  @params {String} [options.format] The image format used in tile requests.
  @params {function} [options.beforeSend] Function to be called before sending the request with the request object given as first parameter. The (modified) request object must be returned.
  @params {String} [options.errorTileUrl] The image to display when requests fail.
  @params {function} [options.onErrorTileSelected] Function to be called when the request has failed and the error tile is selected. 
  @constructor
  **/
 L.PtvLayer.Background = L.TileLayer.extend({
   ...
   options: {
     ...
     /**
      Function to be called when the request has failed and the error tile is selected. 
      @property options.onErrorTileSelected
      @type function
      @default null
      **/
     onErrorTileSelected: null
   },
     ...
     var cb = (function(tile) {
       return function(resp, error) {
         if (!error && resp) {
           ...
           if(self.options && self.options.onErrorTileSelected && typeof self.options.onErrorTileSelected === "function" ) {
               self.options.onErrorTileSelected();
           } else {
               console.log(error.errorMessage);
           }
         }
